<template>
  <div class="space-y-6">
    <div ref="headerRef" class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Salary Sheet</h1>
        <p class="text-slate-500 dark:text-slate-400">Manage and upload salary sheet documents</p>
      </div>
      <button v-if="authStore.hasPermission('salary_sheet_create') || authStore.isSuperAdmin" 
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-32 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <p class="mt-6 text-slate-400 font-black uppercase tracking-widest text-[10px]">Loading Salary Sheets...</p>
    </div>
    ...
    <DataTable v-else
      ...
      <template #actions="{ row }">
        <div class="flex items-center gap-2">
          <button v-if="authStore.hasPermission('salary_sheet_download') || authStore.isSuperAdmin" 
          ...
          <button v-if="authStore.hasPermission('salary_sheet_edit') || authStore.isSuperAdmin" 
          ...
          <button v-if="authStore.hasPermission('salary_sheet_delete') || authStore.isSuperAdmin" 
    ...
    <!-- Upload/Edit Modal -->
    <Modal :show="showModal" :title="isEditing ? 'Edit Salary Sheet' : 'Upload Salary Sheet'" @close="showModal = false" maxWidth="2xl">
    ...
    <ConfirmModal 
      ...
      title="Delete Salary Sheet Document" 
...
import { salarySheetService, companyService } from '@/services/api';
...
const fetchDocuments = async (page = 1) => {
  loading.value = true;
  try {
    const res = await salarySheetService.getAll({
      page,
    ...
    console.error('Failed to fetch salary sheets', err);
...
const handleUpload = async () => {
    ...
    try {
        ...
        await salarySheetService.upload(formData);
        notificationStore.success('Salary Sheet uploaded successfully');
...
const handleUpdate = async () => {
    ...
    try {
        ...
        await salarySheetService.update(editingItem.value.id, formData);
...
const deleteDocument = async () => {
    ...
    deleting.value = true;
    try {
        await salarySheetService.delete(itemToDelete.value.id);
...
const downloadDocument = async (doc) => {
    try {
        const response = await salarySheetService.download(doc.id);
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', doc.file_name || 'salary_sheet.pdf');
