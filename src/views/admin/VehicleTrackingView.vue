    ...
    <DataTable v-if="!loading"
      ...
      <template #actions="{ row }">
        <div class="flex items-center gap-3">
          ...
          <button v-if="authStore.hasPermission('vehicle_status') || authStore.isSuperAdmin" @click="confirmStatusToggle(row)" class="p-1 transition-colors" :class="row.is_active ? 'text-green-500 hover:text-red-500' : 'text-red-500 hover:text-green-500'" :title="row.is_active ? 'Deactivate' : 'Activate'">
