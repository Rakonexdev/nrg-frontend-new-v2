<template>
  <div class="min-h-screen flex items-center justify-center bg-slate-50 p-6 relative overflow-hidden font-outfit">
    <!-- Background Elements -->
    <div class="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
        <div class="absolute -top-[20%] -right-[10%] w-[600px] h-[600px] bg-blue-500/10 blur-[120px] rounded-full"></div>
        <div class="absolute -bottom-[20%] -left-[10%] w-[600px] h-[600px] bg-indigo-500/10 blur-[120px] rounded-full"></div>
    </div>
    ...
    <div class="w-full max-w-md relative z-10 transition-all duration-700 transform" :class="loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'">
        ...
        <div class="flex flex-col items-center mb-10">
            ...
            <p class="text-slate-500 mt-2 font-medium">Empowering your HR workforce</p>
        </div>

        <!-- Card -->
        <div class="bg-white/70 backdrop-blur-2xl p-10 rounded-[2.5rem] border border-white shadow-2xl shadow-slate-200/50">
            <form @submit.prevent="handleLogin" class="space-y-6">
                <div v-if="authStore.error" class="p-4 bg-rose-50 border border-rose-100 rounded-xl text-rose-600 text-sm font-bold flex items-center gap-3">
                ...
                <div class="space-y-2">
                    <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Work Email</label>
                    <div class="relative group">
                        ...
                        <input v-model="form.email" type="email" required placeholder="name@company.com" 
                               class="w-full pl-11 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-bold" />
                    </div>
                </div>

                <div class="space-y-2">
                    <div class="flex items-center justify-between ml-1">
                        <label class="text-xs font-black text-slate-400 uppercase tracking-widest">Secret Key</label>
                ...
                <div class="flex items-center gap-3 ml-1">
                    ...
                    <label for="remember" class="text-sm font-bold text-slate-600 cursor-pointer select-none">Keep me logged in</label>
                </div>
